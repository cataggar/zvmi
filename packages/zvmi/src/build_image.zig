...
pub fn build(
    ...
    if (options.skip_iso_rootfs) {
        ...
        try source_tree.pruneToContainerRootfsAndBootAssets(allocator, &container_image);
    }
...
fn isRequiredBootAssetPath(path: []const u8) bool {
...
const MergedSourceTree = struct {
    ...
    fn pruneToContainerRootfsAndBootAssets(
        ...
        for (self.entries, 0..) |entry, read_index| {
            const keep = container_image.get(entry.path) != null or isRequiredBootAssetPath(entry.path);
            if (keep) {
...
const qemu_boot_smoke_disk_size: u64 = 4 * 1024 * mib;
...
fn runGen2QemuBootSmoke(
    ...
    var timed_out = false;
    var stdout = try allocator.alloc(u8, 0);
    errdefer allocator.free(stdout);
    var stderr = try allocator.alloc(u8, 0);
    errdefer allocator.free(stderr);

    const maybe_result = std.process.run(allocator, io, .{
        .argv = &.{
            prereqs.qemu_path,
            "-M",
            "q35",
        ...
        .timeout = .{
            .duration = .{
                .clock = .awake,
                .raw = .fromSeconds(qemu_boot_smoke_timeout_seconds),
            },
        },
    }) catch |err| switch (err) {
        error.Timeout => blk: {
...
test "build-image opportunistically boot-smokes a provisioned Gen2 raw image under QEMU" {
    ...
    if (!serialOutputShowsKernelBoot(qemu.serial_output)) {
        std.debug.print(
            "QEMU boot smoke test did not reach kernel serial output (timed_out={})\nserial output:\n{s}\nqemu stderr:\n{s}\nqemu stdout:\n{s}\n",
            .{ qemu.timed_out, qemu.serial_output, qemu.stderr, qemu.stdout },
        );
    }
