//! `zvmi build-image --iso <file.iso> --container <oci-layout> --generation 1|2 --size <size> -o <output.{raw|vhd}> [--verity]`
...
pub fn run(gpa: std.mem.Allocator, io: std.Io, args: []const []const u8) u8 {
    ...
    while (i < args.len) : (i += 1) {
        ...
        if (std.mem.eql(u8, arg, "--iso")) {
            ...
            return fail("usage: zvmi build-image --iso <file.iso> --container <oci-layout> --generation 1|2 --size <size> -o <output.{{raw|vhd}}> [-O raw|vhd] [--rootfs-path <path>] [--esp-size <size>] [--verity] [--dry-run] [-v]", .{});
        } else {
